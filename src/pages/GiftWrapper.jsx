...
const GiftWrapper = () => {
  ...
  useEffect(() => {
    const checkCountdown = () => {
      const targetDate = new Date('2025-08-31T11:30:00')
      const now = new Date()
      if (now >= targetDate) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-pink-100 to-purple-100 p-4">
      ...
      <AnimatePresence>
        {showMessage && (
          ...
          >
            <p className="text-pink-600">Hadiah belum bisa dibuka sampai 1 September 2025!</p>
      ...
      {!countdownFinished && (
        <div className="mt-8 p-4 bg-white bg-opacity-80 rounded-lg shadow-md max-w-md text-center">
          ...
          <p className="text-xl font-bold text-purple-600">1 September 2025</p>
        </div>
      )}
    </div>
  )
